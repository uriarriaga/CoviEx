...
from app.funciones import sendWebexMsg, sendSMS, createJWT, generarWebex, existeWebex, agendarWebex, hostJoined, cronSMS, sendAgendaSMS, webexURL
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.password == form.password.data:
            ...
            flash(
                'Login requested for user login Unsuccesful. Plese check username and password')
...
def widget():
    ...
    if invitado.expirationTime <= datetime.utcnow().timestamp():
        ...
        return render_template('widgetexpired.html', title='widget')
    if not hostJoined(invitado.SIP.split("@")[0]) or terminosAceptados == "False":
        return render_template('widgetLobby.html', title='widget')
    JWToken = createJWT(invitado.expirationTime, token)
    return render_template('llamadaSDK.html', title='widget', token=JWToken, SIP=invitado.SIP, redirect=webexURL(invitado.SIP.split("@")[0]))
        ...
        if formusr.admin.data != 0 or formusr.ad.data != 0 or formusr.cp.data != 0:
            ...
            if existeWebex(formusr.email.data) or  formusr.admin.data == 1 or formusr.cp.data == 1:
                ...
                try:
                    ...
                    userx = User.query.filter_by(
                        email=formusr.email.data).first()
                    try:
...
def insertdata():
    ...
    if action == "update":
        ...
        familiares_db = db.session.query(
            Familiar).filter_by(id_paciente=paciente_id)

        for familiar in familiares_db:

            db.session.query(Familiar).filter(
                Familiar.id_paciente == paciente_id).delete()
        ...
        paciente_db = db.session.query(Paciente).filter_by(
            email=paciente.email).first()
...
def capture():
    if not current_user.capturista:
        return redirect(url_for('login'))
    formv = userForm()
    fam = Familiar(nombre="pruebamemo", celular="7222849367",
                   email="mr.memo@gmail.com", id_paciente=1)
    pac = Paciente(nombre="pruebamemo", celular="7222849367",
                   email="mr.memo@gmail.com")
    # db.session.add(fam)
    # db.session.commit()
...
def getfamiliares():
    ...
    for familiar in familiares:
        ...
        list += "{" + '"' + "nombre" + '"'+":" + '"' + familiar.nombre + '"' + "," + '"' + "celular" + '"'+":" + '"' + familiar.celular + '"' + "," + '"' + "email" + '"'+":" + \
            '"' + familiar.email + '"' + "," + '"' + "id" + '"'+":" + '"' + \
                str(familiar.id) + '"' + "," + '"' + "id_paciente" + \
            '"'+":" + '"' + str(familiar.id_paciente) + '"' + "},"
    list = list[:-1]
    list += "]}"
...
def getusuarios():
    ...
    for usuario in usuarios:
        # print(str(familiar.nombre))
        list += "{" + '"' + "username" + '"'+":" + '"' + usuario.username + '"' + "," + '"' + "email" + '"'+":" + '"' + usuario.email + '"' + "," + '"' + "admin" + '"'+":" + '"' + str(usuario.admin) + '"' + "," + '"' + "id" + '"'+":" + '"' + str(
            usuario.id) + '"' + "," + '"' + "Medico" + '"'+":" + '"' + str(usuario.atenciondomiciliaria) + '"' + "," + '"' + "Capturista" + '"'+":" + '"' + str(usuario.capturista) + '"'  "," + '"' + "password" + '"'+":" + '"' + usuario.password + '"'+"},"
    list = list[:-1]
    list += "]}"
...
def llamada():
    ...
    nombre = call["name"]
    tipo = call["tipo"]
    print("EMAIL   -----  >" + email + "nombre   ------   >" +
          nombre + "celulares   - - - -   >" + str(celulares))
...
def agendarllamada():
    ...
    if tipo == "1":
        ...
        SIP = agendarWebex(
            celulares, email, "Atencion domiciliaria " + nombre, utctime)
        ...
        if tipo == "2":
            ...
            SIP = agendarWebex(
                celulares, email, "Informe Medico " + nombre, utctime)
        ...
        if tipo == "3":
            ...
            SIP = agendarWebex(
                celulares, email, "TeleVisita" + nombre, utctime)
